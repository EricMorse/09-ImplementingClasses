...
class Point(object):
    ...
    def move_to(self, x, y):
        distx = self.x - x
        disty = self.y - y
        self.dist = self.dist + math.sqrt(distx**2 + disty**2)
        self.x = x
        self.y = y
    ...
    def get_distance_from(self, p2):
        distx = abs(p2.x - self.x)
        disty = abs(p2.y - self.y)
        return math.sqrt(distx**2 + disty**2)

    def get_distance_from_start(self):
        distx = abs(self.x - self.startx)
        disty = abs(self.y - self.starty)
        return math.sqrt(distx**2 + disty**2)
    ...
    def closer_to(self, p2, p3):
        distx2 = abs(self.x - p2.x)
        disty2 = abs(self.y - p2.y)
        dist2 = math.sqrt(distx2**2 + disty2**2)
        distx3 = abs(self.x - p3.x)
        disty3 = abs(self.y - p3.y)
        dist3 = math.sqrt(distx3**2 + disty3**2)
        if dist2 <= dist3:
            return p2
        else:
            return p3
    ...
    def halfway_to(self, p2):
        halfx = (p2.x - self.x)/2
        halfy = (p2.y - self.y)/2
        return Point(self.x + halfx, self.y + halfy)
